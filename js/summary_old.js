...
/**
 * Import data from Airtable CSV export
 */
async function importFromAirtable() {
    ...
    if (!fileInput) {
        ...
        input.type = 'file';
        input.id = 'csvImportFileInput';
        input.accept = '.csv';
        input.style.display = 'none';
        document.body.appendChild(input);
        return importFromAirtable(); // Retry
    }
    ...
    fileInput.onchange = async function(e) {
        ...
        const confirmed = confirm(
            '⚠️ This will REPLACE all current data with data from Airtable.\n\n' +
        ...
        try {
            ...
            const result = await CSVImporter.importCSV(file, false);
            
            if (result.success) {
                alert(`✓ ${result.message}\n\nTeam member names were looked up from email addresses.`);
                loadUseCases(); // Reload to show imported data
            } else {
                alert('Import failed: ' + result.message);
            }
